...
// and the 2nd node will be the parent of the 5th and 6th nodes. In a specific kind of binary heap, the binary min heap,
...
// When adding a new node to a binary min heap, 
//it could be that we violate the property of the heap whereby every node is of lower value than its children. 
//Thus whenever we insert into a binary min heap, we must compare the inserted node to its parent, 
...
//Because we can only remove from the end of the array we swap the position of the last node and the root node and then remove the now-last node from the heap. 
...
BinaryHeap.prototype.removeRoot = function () {
    // TODO: Your code here
    //swap the root and the last element and remove the last element
    var root = this._heap[0]
    this._heap[0]=this._heap[this._heap.length-1]
    this._heap[this._heap.length-1]=root
    this._heap.pop()
    //chack if the newroot is less than its immediate children
